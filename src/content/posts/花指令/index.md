...
- 正常流程：经过一遍计算后跳转不成立，又return到判断之前，再计算，再判断一次就成立，成功跳转；
- 但ida分析第一遍跳转不成立就不会分析内部的了；
...
> **mprotect**函数是Linux系统中用于修改进程内存页保护属性的系统调用。
>
> 通过这个函数，可以控制某个内存区域的读、写、执行权限。
>
> `mprotect`函数的原型如下：
>
> ```c
...
> ```
>
> - **addr**：需要修改权限的内存区域的起始地址，必须是系统页大小的整数倍。
>
> - **len**：需要修改权限的内存区域的长度，通常也是页大小的整数倍。
>
> - **prot**：指定内存区域的新权限，可以是以下几个值的组合： 
>
>   `PROT_READ`：可读 （1）
>
>   `PROT_WRITE`：可写  （2）
...
>   `PROT_NONE`：不可访问
>
>    **值相加即代表修改后的权限，类似`chmod -777`**

- 通过调用`mprotect`函数将**原来一般不可写的函数段地址**改为**可写**（w），从而修改地址中的字节码，将原来的垃圾字节修改成正常的（比如修改`jmp`的地址）；

- 这样修改前的显式字节不能被反编译了；

修改方法：

- 关注`mprotect`的地址，并在`mprotect`附近寻找函数或加密代码，一般就在附近加密；
- 逆向加密的地址对应的字节码
- 修改加密字节，再反编译
...
![image-20251127155939019](./image-20251127155939019.png)

238b中调用了mprotect，-7修改为rwx，改完内存后再改为rx；

![image-20251127155951151](./image-20251127155951151.png)

![image-20251127155959321](./image-20251127155959321.png)

a1就是指向E9后一位的指针，所以改的是jmp的跳转地址，解密异或，再patch地址即可；
