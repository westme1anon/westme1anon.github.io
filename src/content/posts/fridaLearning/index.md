...
#### 原理：**方法替换（`ArtMethod` 指针替换）**

`Java.use()` / `Java.choose()` 这套 API 本质是操作 ART 虚拟机（解释`.dex`字节码的虚拟机）内部结构：

```
ArtMethod {
    declaring_class_   → Class*
    access_flags_      → uint32_t
    dex_method_index_  → uint32_t
    entry_point_from_quick_compiled_code_  ← 关键字段
}
```

Hook 流程：

1. 找到目标方法的 `ArtMethod` 对象
2. **备份**原始 `entry_point_from_quick_compiled_code_`（即编译后的机器码入口）
3. **替换**为 Frida 生成的 trampoline stub 地址
4. stub 执行你的 JS 回调，再跳回原始入口（`callOriginal()`）

所以 Java 层本质是**换函数指针**，换掉结构体中的入口指针。
...
### Native层Hook原理：**Inline Hook**

`Interceptor.attach(address, ...)` 使用的是 inline hook：
...
Hook后：
  [目标地址]  B  frida_trampoline   ← 写入跳转指令（4字节/ARM64）
              ADD X2, X3, X4        ← 被覆盖的指令搬到 trampoline 执行
              RET
...
- ARM64 下写入 `B <offset>`（4字节），x86 下写入 `JMP rel32`（5字节）
- 被覆盖的指令需要**重定位**（relocate），因为 PC 相对寻址的指令搬到新地址后偏移变了
- Frida 使用 **Stalker**（代码追踪引擎）辅助处理复杂重定位

#### `Interceptor.replace()` 则更激进

直接把函数头替换为跳转到你的完整替代函数，不保留原始流程（除非你手动调用 `NativeFunction` 去调原始备份）。
